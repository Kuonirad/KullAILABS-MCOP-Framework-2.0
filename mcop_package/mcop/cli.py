...
def cmd_solve(args):
    ...
    if args.output:
        try:
            # Security: Use 'x' mode (exclusive creation) unless forced, to prevent
            # accidental overwrites. This is a Time-of-Check to Time-of-Use (TOCTOU)
            # safe way to handle file creation.
            mode = 'w' if args.force else 'x'

            # Additional path check (optional but good practice)
            # Ensure we are not writing to a directory
