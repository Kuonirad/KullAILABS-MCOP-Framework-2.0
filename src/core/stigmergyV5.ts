...
import { ContextTensor, PheromoneTrace, ResonanceResult, StigmergyConfig } from './types';
...
export class StigmergyV5 {
  ...
  // Optimized magnitude calculation: O(N)
  ...
  // O(N) but avoids redundant sqrt calls if magnitudes are provided
  ...
  recordTrace(context: ContextTensor, synthesisVector: number[], metadata?: Record<string, unknown>): PheromoneTrace {
    ...
    // Security: Use crypto.randomUUID() for cryptographically strong IDs
    ...
    const trace: PheromoneTrace = {
      ...
      synthesisVector,
      weight,
      magnitude: contextMag, // Cache the magnitude for faster resonance queries
      metadata,
  ...
  getResonance(context: ContextTensor): ResonanceResult {
    ...
    // 1. Iterate with simple loop (faster than forEach/map)
    // 2. Use cached magnitude from trace (avoids O(N) calc + sqrt)
    // 3. Inline dot product (avoids function call overhead)
    for (let t = 0; t < traceCount; t++) {
      const trace = this.traces[t];
      const tContext = trace.context;

      // Use cached magnitude if available, otherwise calculate it (fallback safety)
